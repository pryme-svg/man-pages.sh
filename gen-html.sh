...
#        echo "$whole" > "$1/$(basename "$f").html"
        ...
        whole=$(gen_page "$body" "title:$title($section)" "source:$src\ndate:$(date -I)" "man_header.html" "man_footer.html")
        echo "$whole" > "$1/$(basename "$f").html"

        echo "$1/$(basename "$f").html" # comment for testing
        ...
        if [ "$(echo "$toc" | grep "\S" | wc -l)" -lt 1 ]; then # if no matches
